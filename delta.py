...
def render_delta_contracts_view():
    ...
    # ── COLUMN 1: Left white paper viewport ─────────────────────────────────
    with doc_col1:
        combined_left = "\n".join(left_html_rows)
        st.markdown(
            f'<div class="paper-viewport">{combined_left}</div>',
            unsafe_allow_html=True
        )

    # ── COLUMN 2: Centre mini-map visual track ───────────────────────────────
    with map_col:
        combined_minimap = "\n".join(minimap_html)
        st.markdown(
            f'<div class="minimap-track">{combined_minimap}</div>',
            unsafe_allow_html=True
        )

    # ── COLUMN 3: Right white paper viewport ────────────────────────────────
    with doc_col3:
        combined_right = "\n".join(right_html_rows)
        st.markdown(
            f'<div class="paper-viewport">{combined_right}</div>',
            unsafe_allow_html=True
        )
