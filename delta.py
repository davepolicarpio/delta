...
def inject_luxury_system_css():
    st.markdown("""
        <style>
            ...
            /* Global Canvas: Light Neutral Gray Desktop */
            .stApp {
            ...
            /* Suppress Native Streamlit Elements */
            ...
            /* Structural Card Cells for Perfect Row Alignment */
            .doc-cell {
            ...
            /* Inline Diff Visual Style Decorators */
...
def load_staged_matrices(uploaded_files):
    for file in uploaded_files:
        if file.name not in st.session_state.uploaded_files_data:
            ...
            if file.name.endswith('.pdf'): parsed_text = parse_pdf(bytes_data)
            elif file.name.endswith('.docx'): parsed_text = parse_docx(bytes_data)
            else: continue
...
def compute_fuzzy_alignment_matrix(left_paras, right_paras, threshold=0.45):
    ...
    for i, lp in enumerate(left_paras):
        ...
        if best_ratio >= threshold and best_j is not None:
            ...
            if best_ratio > 0.98 and lp == right_paras[best_j]: alignment_opcodes.append(('equal', i, i, best_j, best_j))
            else: alignment_opcodes.append(('replace', i, i, best_j, best_j))
        else:
...
def generate_advisory_trace_text(text1, text2, signature):
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'replace':
            o = " ".join(text1.split()[i1:i2]); n = " ".join(text2.split()[j1:j2])
...
def compute_token_diff_html(text1, text2):
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        elif tag == 'replace': out1.append(f'<span class="del-token">{w1}</span>'); out2.append(f'<span class="add-token">{w2}</span>')
...
def export_landscape_docx(left_paras, right_paras, title_left, title_right, alignment_opcodes, hash_l, hash_r):
    ...
    hdr_cells[0].text = f"BASELINE ({title_left})"; hdr_cells[1].text = f"COUNTERPART ({title_right})"; hdr_cells[2].text = "SMART DELTA EVALUATION"
    
    for tag, i1, _, j1, _ in alignment_opcodes:
        row = table.add_row()
        if tag == 'equal':
            row.cells[0].text = left_paras[i1]; row.cells[1].text = right_paras[j1]; row.cells[2].text = "No variance detected."
        elif tag == 'replace':
            p1 = row.cells[0].paragraphs[0]; p2 = row.cells[1].paragraphs[0]
            ...
            for w_tag, w_i1, w_i2, w_j1, w_j2 in m_words.get_opcodes():
                ...
                if w_tag == 'equal': p1.add_run(w1_str); p2.add_run(w2_str)
                else:
            ...
            trace = generate_advisory_trace_text(left_paras[i1], right_paras[j1], extract_clause_signature(left_paras[i1]))
...
def export_landscape_pdf(left_paras, right_paras, title_left, title_right, alignment_opcodes, hash_l, hash_r):
    # PDF compilation logic preserved exactly as requested
    doc = fitz.open()
    ...
    for tag, i1, _, j1, _ in alignment_opcodes:
        ...
        if tag == 'equal':
            ...
            trace = generate_advisory_trace_text(left_paras[i1], right_paras[j1], extract_clause_signature(left_paras[i1]))
            page.insert_textbox(fitz.Rect(c3_x, y + 5, c3_x + col_w, y + 65), f"Eval: {trace}\nAction: {st.session_state.get(f'act_mod_{i1}_{j1}', 'Unassigned')}", fontsize=8, color=(0.1, 0.1, 0.1))
            y += 75
        elif tag == 'delete':
...
def render_premium_landing_view():
    ...
    if uploaded_files:
        ...
        for idx, filename in enumerate(st.session_state.file_order):
            ...
            with col1: st.markdown(f'<p style="font-size:13px; color:#1A1A1A; padding-top:8px;">◼ {filename}</p>', unsafe_allow_html=True)
            with col2:
                roles = ["Standard Template", "v1: Baseline", "v2: Counter", "v3: Counter", "v4: Counter"]
                st.session_state.file_roles[filename] = st.selectbox(f"Classification_{filename}", roles, index=roles.index(st.session_state.file_roles.get(filename, "v1: Baseline")), label_visibility="collapsed", key=f"stage_role_{filename}")
...
# ==========================================
# BLOCK 8: UI PHASE 2 - REVIEW MATRIX (ROW-LOCKED)
# ==========================================
def render_delta_contracts_view():
    ...
    with t_col1: st.session_state.current_baseline = st.selectbox("Baseline Selection Frame", range(len(ordered_files)), format_func=lambda x: f"BASE // {roles[x]}")
    with t_col2: st.session_state.current_counter = st.selectbox("Counterparty Selection Frame", range(len(ordered_files)), format_func=lambda x: f"CNTR // {roles[x]}", index=min(1, len(ordered_files)-1))
    ...
    with st.sidebar:
        ...
        if template_file:
            ...
            for p in st.session_state.uploaded_files_data[template_file]: st.markdown(f'<div style="font-size:12px; color:#525252; margin-bottom:1rem; line-height:1.5;">{p}</div>', unsafe_allow_html=True)
        else:
    ...
    if st.session_state.comparison_mode == "Standard vs Baseline":
        left_paras, right_paras = (st.session_state.uploaded_files_data[template_file] if template_file else st.session_state.uploaded_files_data[base_file]), st.session_state.uploaded_files_data[base_file]
        ...
        left_paras, right_paras = (st.session_state.uploaded_files_data[template_file] if template_file else st.session_state.uploaded_files_data[base_file]), st.session_state.uploaded_files_data[counter_file]
    ...
    # Core Structural Title Columns
    h_col1, _, h_col2, _ = st.columns([4, 0.5, 4, 3.5])
    with h_col1: st.markdown(f"<p style='font-size:11px; text-transform:uppercase; color:#737373; font-weight:600; margin-bottom:15px;'>{col1_title}</p>", unsafe_allow_html=True)
    with h_col2: st.markdown(f"<p style='font-size:11px; text-transform:uppercase; color:#737373; font-weight:600; margin-bottom:15px;'>{col2_title}</p>", unsafe_allow_html=True)

    # ---------------------------------------------------------
    # ROW-LOCKED EXECUTION LOOP (Fixes all vertical alignment)
    # ---------------------------------------------------------
    change_index = 1
    for idx, (tag, i1, _, j1, _) in enumerate(alignment_opcodes):
        # We generate a full Streamlit layout row for EACH paragraph mapped.
        r_col1, r_col2, r_col3, r_col4 = st.columns([4, 0.5, 4, 3.5])
        
        # 1. LEFT DOCUMENT CELL
        with r_col1:
            if tag == 'equal':
                ...
                st.markdown(f'<div class="doc-cell"><span class="trace-flag">▲ Modified</span><p class="stream-paragraph">{h1}</p></div>', unsafe_allow_html=True)
            elif tag == 'delete':
                st.markdown(f'<div class="doc-cell"><span class="trace-flag" style="color:#991B1B; border-bottom-color:#FEE2E2;">◼ Omitted</span><p class="stream-paragraph"><span class="del-token">{left_paras[i1]}</span></p></div>', unsafe_allow_html=True)
            elif tag == 'insert':
        ...
        with r_col3:
            if tag == 'equal':
                ...
                st.markdown(f'<div class="doc-cell"><span class="trace-flag">▲ Modified</span><p class="stream-paragraph">{h2}</p></div>', unsafe_allow_html=True)
            elif tag == 'delete':
                st.markdown('<div class="doc-cell empty-cell"></div>', unsafe_allow_html=True)
            elif tag == 'insert':
                st.markdown(f'<div class="doc-cell"><span class="trace-flag" style="color:#065F46; border-bottom-color:#D1FAE5;">◆ Insertion</span><p class="stream-paragraph"><span class="add-token">{right_paras[j1]}</span></p></div>', unsafe_allow_html=True)

        # 4. ACTION SIDEBAR (Locked to this specific row)
        with r_col4:
            if tag == 'equal':
                st.write("") # Spacer to maintain grid flow
            elif tag == 'replace':
                ...
                st.markdown(f'<div class="advisory-panel" style="border-left: 3px solid #FCA5A5;"><div class="advisory-header"><span style="color:#991B1B; margin-right:5px;">Δ {change_index}</span> CHANGED // {sig}</div><p style="color:#525252; font-size:11px; margin:0 0 10px 0;">{trace}</p></div>', unsafe_allow_html=True)
                ...
                unique_id = f"del_{i1}"
                sig = extract_clause_signature(left_paras[i1])
                st.markdown(f'<div class="advisory-panel" style="border-left: 3px solid #EF4444;"><div class="advisory-header"><span style="color:#EF4444; margin-right:5px;">- {change_index}</span> OMITTED // {sig}</div><p style="color:#525252; font-size:11px; margin:0 0 10px 0;">Omission Directive: The baseline provision is completely absent.</p></div>', unsafe_allow_html=True)
                st.radio("Action Protocol", ["✓", "⚠", "⇄", "✕"], key=f"act_{unique_id}", horizontal=True, label_visibility="collapsed", index=None)
                st.text_input("Comments", key=f"note_{unique_id}", placeholder="Comments", label_visibility="collapsed")
                st.markdown("<div style='margin-bottom:16px;'></div>", unsafe_allow_html=True)
                change_index += 1
                ...
                unique_id = f"ins_{j1}"
                sig = extract_clause_signature(right_paras[j1])
                st.markdown(f'<div class="advisory-panel" style="border-left: 3px solid #10B981;"><div class="advisory-header"><span style="color:#10B981; margin-right:5px;">+ {change_index}</span> INJECTED // {sig}</div><p style="color:#525252; font-size:11px; margin:0 0 10px 0;">Structural Insertion: New provision block missing from baseline.</p></div>', unsafe_allow_html=True)
                st.radio("Action Protocol", ["✓", "⚠", "⇄", "✕"], key=f"act_{unique_id}", horizontal=True, label_visibility="collapsed", index=None)
                st.text_input("Comments", key=f"note_{unique_id}", placeholder="Comments", label_visibility="collapsed")
                st.markdown("<div style='margin-bottom:16px;'></div>", unsafe_allow_html=True)
                change_index += 1
    ...
    with b_col1:
        ...
        if "Word" in target_format:
            ...
            mime_type, extension = "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "docx"
        else:
        ...
        st.download_button(label="📥 Export Review Matrix", data=export_bytes, file_name=f"DELTA_Matrix_{datetime.now().strftime('%Y%m%d')}.{extension}", mime=mime_type)
    with b_col2:
        st.write("<div style='height:1px;'></div>", unsafe_allow_html=True)
        if st.button("Terminate Session"): st.session_state.processing_complete = False; st.rerun()
...
def main():
    ...
    if not st.session_state.processing_complete: render_premium_landing_view()
    else: render_delta_contracts_view()
